<template>
	<div class="player-wrapper">
		<el-tooltip effect="light" :open-delay="1000" placement="right-start">
			<div slot="content">
				<p>
					当前有<span class="s-c">{{ audioList.length }}</span
					>首歌,点击获取更多
				</p>
			</div>
			<el-button
...
export default {
	name: 'music-player',
	async created() {
		await this.$store.dispatch('songs/getSongList').then(() => (this.loading = false));
	},
	data() {
		return {
			loading: true,
	...
	methods: {
		async refreshSong(e, effectName) {
			if (this.audioList.length >= this.allSong - 10) {
				...
				await this.$store.dispatch('songs/getSongDetail').then(() => (this.loading = false));
			}
		},
	},
	computed: {
