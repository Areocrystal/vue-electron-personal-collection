...
const mutations = {
	...
	SET_AUDIO(state, payload) {
		const {choricSinger, imgUrl, extName, songName, url, albumid, fileName} = payload,
			{audioList} = state;
	...
	SET_DEFAULT_IMG(state, binary) {
		state.defaultCover = `data:image/png;base64,${btoa(
...
const actions = {
	async getSongList({commit, dispatch, state}) {
		const response = await service.getSongs().catch((err) => eventBus.errHandler(err)),
			songHeap = [];
	...
	async getSongDetail({commit, state}) {
		const {songNum} = state,
				...
				state.ind +
				(state.songHashList.length - state.ind < songNum
					? state.songHashList.length - state.ind
					: songNum);
		//一个hash值只能请求一首歌
