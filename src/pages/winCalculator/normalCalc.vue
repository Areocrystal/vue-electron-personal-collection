...
export default {
	...
	methods: {
		...
		point(x) {
			if (/\d+$/g.test(this.expression) && !/\.\d+$/.test(this.expression)) this.expression += x;
		},
		deleteAll() {
			this.expression = '';
			this.result = '';
				...
				if (/\d+$/.test(this.expression) || (this.expression === '' && x === '-')) {
					...
					if (/^-?\d+\.?\d*[+×\-÷]\d+\.?\d*/g.test(this.expression)) {
						if (this.result !== '' && /(-?\d+\.?\d*[+×\-÷]){3,}/g.test(this.expression)) {
							...
							this.result = this.partResult(
								this.expression.match(/-?\d+\.?\d*/g)[0],
								this.expression.match(/\d+\.?\d*/g)[1],
								/[+×\-÷]/.exec(this.expression.substr(1)).toString()
							);
						} else
							this.result = this.partResult(
								this.expression.match(/\d+\.?\d*/g)[0],
								this.expression.match(/\d+\.?\d*/g)[1],
								/[+×\-÷]/.exec(this.expression).toString()
							);
					}
				} else if (/-?\d+\.?\d*[+×\-÷]$/.test(this.expression))
					this.expression = this.expression.replace(/[+×\-÷]$/g, x);
				else if (this.result !== '' && this.expression === '') this.expression = this.result + x;
			} else {
				this.shouldFuc = true;
				if (/^-?\d+\.?\d*[+×\-÷]\d+\.?\d*/g.test(this.expression)) {
			...
			if (this.result) {
				...
				if (st) {
					if (/-?\d+\.?\d*[+*\-/]\d+\.?\d*/g.test(st) && this.shouldFuc) {
						...
						this.result = this.precisionFixed(
							eval(x + eval(/^-?\d+\.?\d*[+*\-/]\d+\.?\d*/.exec(st)) + ')')
						);
...
table {
	...
	font: 18px consolas;
	border-radius: 5% 2%;
	box-shadow: 2px 0 10px 12px rgba(10, 10, 10, 0.6);
