    ...
    <div>
        <transition name="custom-classes-transition" enter-active-class="animated rollIn"
            ...
            <div v-if="tableData.length>0">
                <p class="c-desc">当前个数:&nbsp;<span class="s-c c-desc">{{totalNum}}</span>&nbsp;个</p>
                <transition-group enter-active-class="animated lightSpeedIn"
                    ...
                    <el-row v-for="(v, k) in tableData" :key="tableData.indexOf(v)">
                        <transition-group enter-active-class="animated rotateInUpLeft"
                                          leave-active-class="animated zoomOutDown">
                            <el-tag hit v-for="(v,_k) in tableData[k]"
                                    :key="v.val" :type="tableData[k][_k]['type']"
        ...
        <transition name="create-description-transition"
...
<script>
    ...
    export default {
        ...
        methods: {
            createMatrix() {
                const calcWorker = new Worker(), {size} = this, {numTotal, numRow} = this.necessary;
            ...
            startSelect() {
                ...
                let {rndRow, rndCol} = this, selectedCell;
                ...
                const startTime = +new Date, gapTime = this.necessary.gap * 1000, recycle = () => {
...
<style lang="less" scoped>
    ...
    .el-tag {
        ...
        &:hover {
            ...
            box-shadow: 0 2px 4px rgba(0, 0, 0, .27), 0 0 6px rgba(0, 0, 0, .06);
            cursor: pointer;
        }
    }
