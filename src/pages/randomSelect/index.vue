<template>
  <div slot="selectOne" class="selected-wrapper">
    <el-popover placement="bottom"
                width="50%"
                v-model="createVisible" slot="create">
      <el-form status-icon :model="gameForm" :rules="rules" label-width="75px"
               ref="ruleForm" style="padding-top: 20px;">
        ...
        <el-form-item label="设置时长" prop="gap">
          <el-tooltip class="item" effect="light" :open-delay="1000"
                      content="设置游戏开始后的抽取时间，最短设置2秒，最长设置15秒" placement="bottom">
            <el-input-number v-model="gameForm.gap" :min="1.5" :max="15" :step=".1"
        ...
        <el-form-item>
          ...
          <el-button type="primary" size="mini"
                     @click.prevent="submitForm('ruleForm')">确定
          </el-button>
        </el-form-item>
      </el-form>
    ...
    <matrixEffect :necessary="gameForm" @done="changeState" @minusTotal="changeTotal" ref="matrix"></matrixEffect>
  </div>
</template>
...
<script>
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            next(vm => (vm.$store.commit('EDIT_METANAME', to.meta), vm.$store.commit('EDIT_MENUINDEX', '2-2')))
        },
        data() {
