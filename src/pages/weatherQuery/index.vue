    ...
    <div class="weather-include">
        <el-row v-loading="loadingData" element-loading-text="努力加载中……">
            <el-col :xs="9" :md="8" :lg="7" style="text-align: left;">
                ...
                <el-radio-group v-model="regionPrecision"
                                @change="precisionChange" class="b-s">
            ...
            <el-col :xs="15" :md="16" :lg="17">
                ...
                <el-cascader v-model="selectedOptions" :options="options" :disabled="disableCascader"
        ...
        <el-row ref="graphRoot">
            ...
            <el-col :span="24">
                <weather-chart :adcode="locInfo.adcode||initAdcode" @cascaderState="changeState"></weather-chart>
            </el-col>
        </el-row>
    </div>
</template>
...
<script>
    ...
    export default {
        name: "weather-query",
        components: {
            displayMap, weatherChart,
        },
        beforeRouteEnter(to, from, next) {
            next(vm => (vm.$store.commit('EDIT_METANAME', to.meta), vm.$store.commit('EDIT_MENUINDEX', '3-2')))
        },
        created() {
        ...
        methods: {
            async getAllDistrict(level) {
                await this.$store.dispatch('district/getDistrictInfo', level)
            ...
            findResult(target, layer) {
                return Array.isArray(target) && target.find(item => item.adcode === this.selectedOptions[layer])
            },
            changeHandler() {
                ...
                let regionDetail;
                isPrecision && (
                    regionDetail = findResult(cityDetail.districts, 2)
                );
