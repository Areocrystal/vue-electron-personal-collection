<template>
    <el-card :body-style="{padding:'7px'}" v-loading="loading" element-loading-text="努力获取中……"
    ...
    export default {
        ...
        methods: {
            async getDetailWeather() {
                ...
                await $store.dispatch('weather/getWeatherData', {
                    city: adcode,
                    extensions: 'all'
                }).then(() => {
                    this.drawChart();
            ...
            handlerData() {
                const {weatherMessage} = this, {casts, city, reporttime} = weatherMessage,
                    titleText = `${casts[0].date} —— ${casts[casts.length - 1].date}`;
                const dayData = [], nightData = [];
                casts.forEach(weather => {
                    let tmpDayData = [], tmpNightData = [],
                        {week, date, daytemp, daypower, nighttemp, nightpower, daywind, nightwind, dayweather, nightweather} = weather;
            ...
            drawChart() {
                ...
                const formatSeries = (name, data) => ({
                    ...
                    itemStyle: {
                        ...
                        shadowOffsetY: 5,
                        color: new Charts.graphic.RadialGradient(.4, .3, 1, [{
                            offset: 0,
                ...
                wChart.setOption({
                    backgroundColor: new Charts.graphic.LinearGradient(1, .8, .6, .5,[{
                        offset: 0,
                        color: '#409eff'
                    }, {
                        offset: 1,
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            return `${day.value[2]}<br/>${day.seriesName}&nbsp;：${day.value[day.value.length - 2]}<br/>${night.seriesName}&nbsp;：${night.value[night.value.length - 2]}`;
                        }
                    },
                    legend: {
                        right: 1,
